...
void wifiTask(void *pvParameters) {
    ...
    WiFi.begin(WIFI_SSID.c_str(), WIFI_PASSWORD.c_str());  // Use dynamically loaded credentials
    ...
    setenv("TZ", TIME_ZONE.c_str(), 1);  // Use dynamically loaded time zone
    tzset();
...
void fetchNTPTime() {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (getLocalTime(&timeInfo)) {
            ...
            led.setLedColor(GREEN);  // Green color to indicate successful NTP fetch
